/*
 * linux/arch/arm/mach-mmsp2/usb_eth.c
 *
 * Copyright (C) 2004,2005 DIGNSYS Inc. (www.dignsys.com)
 * Kane Ahn < hbahn@dignsys.com >
 * hhsong < hhsong@dignsys.com >
 *
 * Ethernet driver for the MMSP2 USB client function
 *
 * Based on linux/kernel/arch/arm/mach-sa1100/usb-eth.c
 *
...
#ifndef DMA_NO_COPY
	mmsp2_usb_recv(dmabuf, usb_rsize,
			usb_recv_callback);
#else
	mmsp2_usb_recv(cur_rx_skb->tail, MIN(usb_rsize, skb_tailroom (cur_rx_skb)),
			usb_recv_callback);
#endif
	if (!skb)
		return;
...
#ifndef DMA_NO_COPY
			mmsp2_usb_recv(dmabuf, usb_rsize, usb_recv_callback);
#else
			mmsp2_usb_recv(cur_rx_skb->tail, MIN(usb_rsize, skb_tailroom (cur_rx_skb)), usb_recv_callback);
#endif
	}
}
...
usb_send_callback(int flag, int size)
{
	...
	ret = mmsp2_usb_send(cur_tx_skb->data, cur_tx_skb->len, usb_send_callback);
	if (ret) {
...
usb_eth_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (cur_tx_skb) {
		...
		cur_tx_skb = skb;
		dev->trans_start = jiffies;
		ret = mmsp2_usb_send(skb->data, skb->len, usb_send_callback);
		if (ret) {
			/* If the USB core can't accept the packet, we drop it. */
			dev_kfree_skb(skb);
			cur_tx_skb = NULL;
...
MODULE_PARM_DESC(usb_rsize, "number of bytes in packets from host to mmsp2");
MODULE_PARM(usb_wsize, "1i");
MODULE_PARM_DESC(usb_wsize, "number of bytes in packets from mmsp2 to host");
#endif
...
usb_eth_init(void)
{
	...
	if ( rc == 0 ) {
		 ...
		 pstr = mmsp2_usb_kmalloc_string_descriptor( "MMSP2 USB NIC" );
		 if ( pstr ) {
			  mmsp2_usb_set_string_descriptor( 1, pstr );
			  pd->dev.iProduct = 1;
		 }
		 rc = mmsp2_usb_start();
	}
...
usb_eth_cleanup(void)
{
	...
	mmsp2_usb_stop();
	mmsp2_usb_close();
	if ( (pstr = mmsp2_usb_get_string_descriptor(1)) != NULL )
		kfree( pstr );
#ifndef DMA_NO_COPY
	kfree(dmabuf);
#endif
