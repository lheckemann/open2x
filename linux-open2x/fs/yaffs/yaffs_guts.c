...
const char *yaffs_guts_c_version="$Id: yaffs_guts.c,v 1.1.6.4 2004/03/29 12:09:30 bushi Exp $";
...
yaffs_Object *yaffs_CreateNewObject(yaffs_Device *dev,int number,yaffs_ObjectType type)
{
	...
	if(theObject)
	{
		...
		theObject->st_atime_ = theObject->st_mtime_ =	theObject->st_ctime_ = CURRENT_TIME.tv_sec;		
#else
		theObject->st_atime_ = theObject->st_mtime_ =	theObject->st_ctime_ = CURRENT_TIME;		
#endif
#endif
		switch(type)
		{
...
#else
#if defined(CONFIG_KERNEL_2_5)
		in->st_atime_ = in->st_mtime_ = in->st_ctime_ = CURRENT_TIME.tv_sec;
#else
		in->st_atime_ = in->st_mtime_ = in->st_ctime_ = CURRENT_TIME;
#endif
		in->st_rdev  = rdev;
		in->st_uid   = uid;
		in->st_gid   = gid;
...
int yaffs_UpdateObjectHeader(yaffs_Object *in,const char *name, int force)
{
    ...
    if(!in->fake || force)
    {
		...
		oh->st_uid = in->st_uid;
		oh->st_gid = in->st_gid;
		oh->st_atime_ = in->st_atime_;
		oh->st_mtime_ = in->st_mtime_;
		oh->st_ctime_ = in->st_ctime_;
...
#else
#if defined(CONFIG_KERNEL_2_5)
			in->st_mtime_ = CURRENT_TIME.tv_sec;
#else
			in->st_mtime_ = CURRENT_TIME;
#endif
#endif
		}
				   ...
				   state == YAFFS_BLOCK_STATE_SCANNING; c++)
		{
			...
			else
			{
				    ...
				    tags.objectId == YAFFS_OBJECTID_LOSTNFOUND))
				{
					...
					in->st_uid   = oh->st_uid;
					in->st_gid   = oh->st_gid;
					in->st_atime_ = oh->st_atime_;
					in->st_mtime_ = oh->st_mtime_;
					in->st_ctime_ = oh->st_ctime_;
...
#if defined(CONFIG_KERNEL_2_5)
	if(valid & ATTR_ATIME) obj->st_atime_ = attr->ia_atime.tv_sec;
	if(valid & ATTR_CTIME) obj->st_ctime_ = attr->ia_ctime.tv_sec;
	if(valid & ATTR_MTIME) obj->st_mtime_ = attr->ia_mtime.tv_sec;
#else
	if(valid & ATTR_ATIME) obj->st_atime_ = attr->ia_atime;
	if(valid & ATTR_CTIME) obj->st_ctime_ = attr->ia_ctime;
	if(valid & ATTR_MTIME) obj->st_mtime_ = attr->ia_mtime;
#endif
...
#if defined(CONFIG_KERNEL_2_5)
	attr->ia_atime.tv_sec = obj->st_atime_;	valid |= ATTR_ATIME;
	attr->ia_ctime.tv_sec = obj->st_ctime_;	valid |= ATTR_CTIME;
	attr->ia_mtime.tv_sec = obj->st_mtime_;	valid |= ATTR_MTIME;
#else	
	attr->ia_atime = obj->st_atime_;	valid |= ATTR_ATIME;
	attr->ia_ctime = obj->st_ctime_;	valid |= ATTR_CTIME;
	attr->ia_mtime = obj->st_mtime_;	valid |= ATTR_MTIME;
#endif	
