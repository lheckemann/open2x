...
const char *yaffs_fs_c_version = "$Id: yaffs_fs.c,v 1.1.6.3 2004/03/29 12:09:30 bushi Exp $";
...
static void yaffs_FillInodeFromObject(struct inode *inode, yaffs_Object *obj)
{
	if (inode && obj) 
	{
		...
		inode->i_atime.tv_sec = (time_t)(obj->st_atime_);
		inode->i_atime.tv_nsec = 0;
		inode->i_mtime.tv_sec = (time_t)obj->st_mtime_;
		inode->i_mtime.tv_nsec =0;
		inode->i_ctime.tv_sec = (time_t)obj->st_ctime_;
		inode->i_ctime.tv_nsec = 0;
#else
		inode->i_rdev = obj->st_rdev;
		inode->i_atime = obj->st_atime_;
		inode->i_mtime = obj->st_mtime_;
		inode->i_ctime = obj->st_ctime_;
#endif
...
#if defined(CONFIG_YAFFS_DISABLE_WRITE_VERIFY) && !defined(CONFIG_MTD_NAND_VERIFY_WRITE)
