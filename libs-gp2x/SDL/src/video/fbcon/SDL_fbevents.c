...
/*
 * ghcstop_041124, console(/dev/tty or /dev/tty0)에 framebuffer가 뜰 경우는 이것을 1로 해줘야 한다.
 * console(/dev/tty or /dev/tty0) == fb의 경우인데, x86의 경우이다.
 * 하지만 mmsp2나 다른 임베디드 장비의 경우는 console(/dev/tty or /dev/tty0) != fb이기 때문에
 * vt(virtual terminal)로 키입력을 전환하면 먹지 않게 된다.
 * 그래서 serial console일 경우는 이것을 0으로 해줘야 한다.
 */
...
SDL_StartEventLoop()(SDL_events.c)에서 SDL_KeyboardInit()(SDL_keyboard.c)를 호출한다.

SDL_KeyboardInit()에서 여기 선언되어 있는 this->InitOSKeymap = FB_InitOSKeymap;를 호출한다.
...
dprintf("FB_vgainitkeymaps() => KDGKBENT ioctl return 0\n");
...
int FB_OpenKeyboard(_THIS)
{
	/* Open only if not already opened */
 	if ( keyboard_fd < 0 ) {
		...
		//keyboard_fd = tty0_fd; // ghcstop add
#if FB_VT_OPEN // ghcstop
		if ( tty0_fd < 0 ) {
		...
		if ( (geteuid() == 0) && (current_vt > 0) ) {
 		...
 		if ( keyboard_fd < 0 ) {
			...
			dprintf("kfd<0 ==> open tty\n");
 		}
#ifdef DEBUG_KEYBOARD
		fprintf(stderr, "Current VT: %d\n", current_vt);
#endif
		dprintf("Current VT: %d\n", current_vt);
 		saved_kbd_mode = -1;
#if FB_VT_OPEN // ghcstop
...
#endif
		/* Set up keymap */
		//FB_vgainitkeymaps(keyboard_fd); // ghcstop: 041206 keymap을 밑에서 InitOSKeymap()에서
		// 직접 세팅하기 때문에 이녀석은 필요없다. 키보드는 단지 키 입력을 받을 뿐이고 키보드에서
		// 키맵을 읽어오지 않도록 한다.
...
dprintf("handle_keyboard(): read pre\n"); // ghcstop
	nread = read(keyboard_fd, keybuf, BUFSIZ);
dprintf("handle_keyboard(): read aft\n"); // ghcstop	
	...
	for ( i=0; i<nread; ++i ) {
		...
		dprintf("scancode = 0x%02x, kbuf[%d] = 0x%02x\n", scancode, i, keybuf[i]);
...
//dprintf("FB_PumpEvents: select\n");		
		if ( select(max_fd+1, &fdset, NULL, NULL, &zero) > 0 ) {
			if ( keyboard_fd >= 0 ) {
				dprintf("FB_PumpEvents: kbd select\n");		// ghcstop
...
void FB_InitOSKeymap(_THIS)
{
	...
	keymap[3] =  SDLK_CRTL_C	;// ghcstop 041206 add: keymap을 뜯어고치면서 ctrl+c도 key event로 먹게 만든다.
