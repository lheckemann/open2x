...
// ghcstop: 나중에 debugging할때늕 SDL_SetError를 printf로 바꿔서 해줄 것 
//#define DEBUG_VIDEO // ghcstop, 모드 디버깅시는 풀어줄 것 
...
int SDL_VideoInit (const char *driver_name, Uint32 flags)
{
	...
	if ( video == NULL ) {
        ...
        ghcd(("No available video device")); 
		return(-1);
	}
	current_video = video;
