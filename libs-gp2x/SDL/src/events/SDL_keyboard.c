...
#if 0 // ghcstop 041206: keyboard event routine change
...
/* ghcstop 041206: keyboard event routine change
keymap[3] =  SDLK_CRTL_C	;// ghcstop 041206 add: keymap을 뜯어고치면서 ctrl+c도 key event로 먹게 만든다.

key press event만먹게 만들었음. key release는 없앴음. 왜냐? key 연속 입력은 빼야징....^^

keymap[3] =  SDLK_CRTL_C	;// ghcstop 041206 add: keymap을 뜯어고치면서 ctrl+c도 key event로 먹게 만든다.
그리고, keymap의 3번인 ctrl+c가 들어올 경우는 약간 속여서 key event를 SDL_QUIT이 먹도록 바꾼다.
*/
