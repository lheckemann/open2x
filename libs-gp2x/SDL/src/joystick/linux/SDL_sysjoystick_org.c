...
int SDL_SYS_JoystickInit(void)
{
	...
	for ( i=0; i<SDL_TABLESIZE(joydev_pattern); ++i ) {
		for ( j=0; j < MAX_JOYSTICKS; ++j ) {
			printf("S111111111111111111\n");
		/*	sprintf(path, joydev_pattern[i], j);( */
			sprintf(path, "/dev/js%d", i);
			...
			if ( stat(path, &sb) == 0 ) {
				...
				printf("S222222222222222222222222222\n");
				fd = open(path, O_RDONLY, 0);
				if ( fd < 0 ) {
					continue;
				}
				...
				printf("S4444444444444444444444444444444\n");
#endif
				printf("S3333333333333333333333333333333\n");
				close(fd);
