...
//#define DEBUG_AUDIO
//#define GHCDEBUG // ghcstop
#ifdef GHCDEBUG
#define ghcd(x) printf("(%s, %d), %s: ", __FILE__, __LINE__, __FUNCTION__); printf x;
#else
#define ghcd(x)
#endif
...
#else
/* Open the audio device for playback, and don't block if busy */
#define USE_BLOCKING_WRITES // ghcstop fix, swl-patch non-blocking error?
...
#ifdef DEBUG_AUDIO
static void Print_AudioSpec(SDL_AudioSpec *spec)
{
	ghcd(("\nfreq     = %d\n", spec->freq));
	ghcd(("format   = 0x%4.4x\n", spec->format));
	ghcd(("channels = %d\n", spec->channels));
...
static void DSP_WaitAudio(_THIS)
{
	...
	if ( frame_ticks ) {
		...
		ghcd(( "Waiting for audio to get ready\n"));
...
static void DSP_PlayAudio(_THIS)
{
	...
	ghcd(( "Wrote %d bytes of audio data\n", written));
}
						...
						SDL_AudioSpec *spec)
{
	...
	frag_spec |= 0x00100000;	/* ghcsto, 16 fragments, 2 fragment를 쓰면 소리가 띡띡 끊긴다 */

	/* Set the audio buffering parameters */
	ghcd(( "Requesting %d fragments of size %d\n",(frag_spec >> 16), 1<<(frag_spec&0xFFFF) ));
	...
	value = spec->freq;
        ...
        // ghcstop: s3c2410에서는 이 부분이 구현안되어 있고, SNDCTL_DSP_SPEED으로 구현되어 있다.
        #if 0 // ghcstop
