...
static int GP2X_InitHWSurfaces(_THIS)
{
  ...
  //senquack - not sure if we need to be calling this, it seems to be called in SDL_video.c in 
  //		SDL_VideoInit()
//  SDL_CursorInit(1);
...
static int GP2X_FlipHWSurface(_THIS, SDL_Surface *surface)
{
  ...
  // Waiting for vblank can be used by the YUV stuff too, so I moved it into a separate function
