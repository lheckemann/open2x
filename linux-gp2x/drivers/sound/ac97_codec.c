...
static int ac97_mixer_ioctl(struct ac97_codec *codec, unsigned int cmd, unsigned long arg)
{
	...
	if (_SIOC_DIR(cmd) == (_SIOC_WRITE|_SIOC_READ)) {
		...
		case SOUND_MIXER_RECSRC: /* Arg contains a bit for each recording source */
			...
			return 0;
		case SOUND_MIXER_EQ_HW:			
			ac97_set_hw_eq(codec,val);    
			return 0;
...
static int wolfson_init05(struct ac97_codec * codec)
{
#define MIC2_ON // Internal Mic

#ifndef MIC2_ON
	codec->codec_write(codec, AC97_GENERAL_PURPOSE,AC97_GP_POP|AC97_GP_MIX);
#else
	codec->codec_write(codec, AC97_GENERAL_PURPOSE,AC97_GP_POP|AC97_GP_MIX|AC97_GP_MS);
#endif

	/* Enable variable rate audio mode */
	codec->codec_write(codec, AC97_EXTENDED_STATUS, AC97_EA_VRA);

//	codec->codec_write(codec, 0x5A, 0x10);	/* MPM Disable */
	codec->codec_write(codec, 0x5A, 0x00);	/* MPM(Mixer Path Mute) Enable for Mic-in, Line-in */
	codec->codec_write(codec, 0x5C, 0xC080); /* AMUTE, HSCP, AMEN */

//	codec->codec_write(codec, AC97_WM97XX_FMIXER_VOL, AC97_MUTE);
	codec->codec_write(codec, AC97_WM97XX_FMIXER_VOL, 0x0000); /* Front Mixer Volume 100 */
	codec->codec_write(codec, 0x74, 0x00);
	return 0;
}

/* WM9711, WM9712 */
#define DEFULT_VOLUME_LINE		0x0	/* -4.5db */	
#define RVOL_MASK 				0
#define LVOL_MASK				8
...
static int wolfson_init11(struct ac97_codec * codec)
{
	...
	/* speaker volume */
	reg = codec->codec_read(codec, 0x02);
	reg &=(~RLVOL_MASK);
	reg |= (DEFULT_VOLUME_LINE << RVOL_MASK) | (DEFULT_VOLUME_LINE << LVOL_MASK);
	codec->codec_write(codec, 0x2, (reg|(1<<6)));
#endif

#ifdef CONFIG_MACH_GP2X	
	//codec->codec_write(codec, 0x02, 0);
	/* disbale input|mono|out */
#if 1	
	reg = codec->codec_read(codec, 0x024);
	...
	reg = codec->codec_read(codec, 0x16);
	reg|=0x100;
...
static void ac97_set_hw_eq(struct ac97_codec *codec,unsigned int Mode ) 
{
	...
	{
		case FLAT:
			codec->codec_write(codec, 0x08, 0x0F4F);
			codec->codec_write(codec, 0x20, 0x2000);
			//codec->codec_write(codec, 0x20, 0x0000);
			codec->codec_write(codec, 0x22, 0x0);
			break;
		case ROCK:
			codec->codec_write(codec, 0x08, 0x8408);
			codec->codec_write(codec, 0x20, 0x2000);
			//codec->codec_write(codec, 0x20, 0x0000);
			codec->codec_write(codec, 0x22, 0x5);
			break;
		case POP:
			codec->codec_write(codec, 0x08, 0x0908);
			codec->codec_write(codec, 0x20, 0x2000);
			//codec->codec_write(codec, 0x20, 0x0000);
			codec->codec_write(codec, 0x22, 0x7);	
			break;
		case JAZZ:
			codec->codec_write(codec, 0x08, 0x1354);
			codec->codec_write(codec, 0x20, 0x2000);
			//codec->codec_write(codec, 0x20, 0x0000);
			codec->codec_write(codec, 0x22, 0x25);	
			break;
		case CLASSICAL:
			codec->codec_write(codec, 0x08, 0x0F42);
			codec->codec_write(codec, 0x20, 0x2000);
			//codec->codec_write(codec, 0x20, 0x0000);
			codec->codec_write(codec, 0x22, 0x3);	
			break;
		case DANCE:
			codec->codec_write(codec, 0x08, 0x8103);
			codec->codec_write(codec, 0x20, 0x2000);
			//codec->codec_write(codec, 0x20, 0x0000);
			codec->codec_write(codec, 0x22, 0x2C);	
			break;
		case HEAVY:
			codec->codec_write(codec, 0x08, 0x0343);
			codec->codec_write(codec, 0x20, 0x2000);
			//codec->codec_write(codec, 0x20, 0x0000);
			codec->codec_write(codec, 0x22, 0x00);	
			break;
		case DISCO:
			codec->codec_write(codec, 0x08, 0x8404);
			codec->codec_write(codec, 0x20, 0x2000);
			//codec->codec_write(codec, 0x20, 0x0000);
			codec->codec_write(codec, 0x22, 0x0);	
			break;
		case SOFT:
			codec->codec_write(codec, 0x08, 0x8418);
			codec->codec_write(codec, 0x20, 0x2000);
			//codec->codec_write(codec, 0x20, 0x0000);
			codec->codec_write(codec, 0x22, 0x0);	
			break;
		case LIVE_3D_ST:
			...
			codec->codec_write(codec, 0x20, 0x2000);
			//codec->codec_write(codec, 0x20, 0x0000);
			codec->codec_write(codec, 0x22, 0x0B);	
			break;
		case HALL:
			codec->codec_write(codec, 0x08, 0x0442);
			codec->codec_write(codec, 0x20, 0x2000);
			//codec->codec_write(codec, 0x20, 0x0000);
			codec->codec_write(codec, 0x22, 0x0B);	
			break;
		default:
			printk("speaker off\n");
			reg = codec->codec_read(codec, 0x02);
			codec->codec_write(codec, 0x2, reg|0x8000);					
	}
}
