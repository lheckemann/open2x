...
#if USE_RANDOM_MAC_ADDR
	{
		/* drivers/char/random.c */
		extern void get_random_bytes(void *buf, int nbytes);
		get_random_bytes((void*)&(dev->dev_addr[0]), 6);
		dev->dev_addr[0] &= 0xfe;    // clear multicast bit
		dev->dev_addr[1] &= 0xfe;    // second byte must be even
	}
#endif
...
#elif defined(CONFIG_MACH_MMSP2_MDK)
	{
		int ioaddr = MDK_ETH_VIO_BASE + 0x300;
		global_dev->irq = SMC_IRQ;

#if 0
		/* 
		 * this is correct, 
		 * but there is(are) unknown bug(s) with GPIO interrupt in MMSP2 CPU.
		 */
		set_external_irq(global_dev->irq, EINT_RISING_EDGE, GPIOPU_DIS);
#else
		set_external_irq(global_dev->irq, EINT_HIGH_LEVEL, GPIOPU_DIS);
#endif
...
#if !defined(CONFIG_ISA) && !defined(CONFIG_MACH_MMSP2_MDK)
