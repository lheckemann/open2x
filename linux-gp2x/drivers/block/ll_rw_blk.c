...
void ll_rw_block(int rw, int nr, struct buffer_head * bhs[])
{
	...
	DPRINTK("##[%s:%d]->%s(): \n", __FILE__,__LINE__,__FUNCTION__);
	DPRINTK1("##[%s:%d]->%s(): rw(%d),nr(%d)\n", __FILE__,__LINE__,__FUNCTION__,rw,nr);
	if (!nr) {
	DPRINTK1("##[%s:%d]->%s(): return\n", __FILE__,__LINE__,__FUNCTION__);
		return;
	}
	...
	for (i = 0; i < nr; i++) {
		...
		switch(rw) {
		case WRITE:
	DPRINTK1("##[%s:%d]->%s(): WRITE\n", __FILE__,__LINE__,__FUNCTION__);
			if (!atomic_set_buffer_clean(bh)) {
				/* Hmmph! Nothing to write */
	DPRINTK1("##[%s:%d]->%s(): goto end_io\n", __FILE__,__LINE__,__FUNCTION__);
				goto end_io;
			}
