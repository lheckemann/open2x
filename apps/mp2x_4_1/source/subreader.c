...
subtitle* sub_read_line_sami(FILE * fd, subtitle * current)
{
    ...
    do
    {
		...
		case 100:
			if (*s == '\0')
				break;
            else if ((!strncasecmp(s, "<sync ", 6)) || (!strncasecmp(s, "</style>", 8)) || (!strncasecmp(s, "</head>", 7)) || (!strncasecmp(s, "<body", 5)))
            {
				printf("Multi Language Check Exit(%d)...\n", baselen);
				baselen = -1;
				state = 0;
				continue;
			}
            else if (!strncasecmp(s, "<style", 6))
            {
				state = 101;
				continue;
			}
		...
		case 101:
			...
			else if ((baselen == 0) && (*s == '.'))
            {
				...
				if(baselen == 0)
					...
					while(baselang[baselen-1] == ' ')
						baselen--;
					baselang[baselen] = '\0';
					printf("Base Language Code: '%s'\n", baselang);
					state = 0;
				}
				continue;
			}
        ...
        case 2:                /* find ">" */
			...
			else if(!strncasecmp(s, "class", 5))
			{
				s += 5;
				while((*s == ' ') || (*s == '=') || (*s == '"') || (*s == '\''))
					s++;
				lang = 1;
				if(!strncasecmp(s, baselang, baselen))
				{
					s += baselen;
					if((*s == ' ') || (*s == '"') || (*s == '\'') || (*s == '>'))
						lang = 0;
        ...
        case 3:                /* get all text until '<' appears */
            if (*s == '\0')
                break;
            else if ((!strncasecmp(s, "<br>", 4)) || (!strncasecmp(s, "<br/>", 4)))
            {
                *p = '\0';
                p = text;
                trail_space(text);
		...
		case 7:
			if(*s == '\0')
				break;
            if (!strncasecmp(s, "</ruby>", 7))
            {
				state = 3;
				s += 7;
				continue;
            }
            else if (!strncasecmp(s, "<sync ", 6))
            {
                state = 3;
				continue;
            }
...
sub_read_line_vplayer(FILE * fd, subtitle * current)
{
    ...
    while (!current->text[0])
    {
        ...
        if (!(current->start = a1 * 360000 + a2 * 6000 + a3 * 100))
            continue;
        /*
         * removed by wodzu p=line; // finds the body of the subtitle for (i=0; i<3; i++){ p=strchr(p,':'); if (p==NULL) break; ++p; } if (p==NULL) { printf("SUB: Skipping incorrect subtitle
         * line!\n"); continue; } 
         */
...
subcp_recode(subtitle * sub)
{
    ...
    while (l)
    {
        ...
        {
                        printf("SUB: error recoding line '%s'.\n", sub->text[l]);
			free(sub->text[l]);
			sub->text[l] = strdup("[Subtitle Error]");
			continue;
...
adjust_subs_time(subtitle * sub, float subtime, float fps, int block, int sub_num, int sub_uses_time)
{
    ...
    if (i)
        for (;;)
        {
            ...
            /*
             * Theory: Movies are often converted from FILM (24 fps) to PAL (25) by simply speeding it up, so we to multiply the original timestmaps by (Movie's FPS / Subtitle's (guessed) FPS) so eg. 
             * for 23.98 fps movie and PAL time based subtitles we say -subfps 25 and we're fine! 
             */
...
sub_filenames(char *path, char *fname)
{
    ...
    int             len,
                    ...
                    i,
                    j;
    char           *sub_exts[] = { "utf", "utf8", "utf-8", "sub", "srt", "smi", "sami", "smil", "rt", "txt", "ssa", "aqt", "jss", "js", "ass", NULL };
    subfn          *result;
    char          **result2;
