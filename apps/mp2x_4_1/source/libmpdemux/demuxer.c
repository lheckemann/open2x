...
// ---- ghcstop_add for data queuing ----, 20050116      
    SDL_DestroyMutex(ds->dsmutex);
    SDL_DestroyCond(ds->dscond);
// ---- ghcstop_add end for data queuing ----, 20050116      
...
// ---- ghcstop_add for data queuing ----, 20050116  
  ds->dsmutex = SDL_CreateMutex();
  ds->dscond = SDL_CreateCond();
  ds->abort_request = 0;  
  ds->nodata = 0;  
...
void ds_queue_abort(demux_stream_t *q)
{
    SDL_LockMutex(q->dsmutex);

    q->abort_request = 1;
    
    SDL_CondSignal(q->dscond);

    SDL_UnlockMutex(q->dsmutex);
}
...
demuxer_t* new_demuxer(stream_t *stream,int type,int a_id,int v_id,int s_id){
  ...
  d->dmutex = SDL_CreateMutex(); // demux_fill_buffer locking¿ë
  d->rhd    = 0;
...
// ghcstop add 050227 ====================    
    SDL_DestroyMutex(demuxer->dmutex);
// ghcstop add 050227 ====================    
    
    free(demuxer);
}

#ifndef DFTE // ghcstop: original
...
// ---- ghcstop_add for data queuing ----, 20050116                  
SDL_CondSignal(ds->dscond);
SDL_UnlockMutex(ds->dsmutex);    
// ---- ghcstop_add end for data queuing ----, 20050116          
}
...
//  printf("demux->type=%d\n",demux->type);
  ...
  switch(demux->type){
    ...
    case DEMUXER_TYPE_AVI: return demux_avi_fill_buffer(demux);
    	...
    	{// ghcstop
    		int aa;
    		if(ds == demux->video) printf("video_demux_fill_buffer(): demux_avi_fill_buffer\n");
    		else printf("audio_demux_fill_buffer(): demux_avi_fill_buffer\n");
    		aa = demux_avi_fill_buffer(demux);
    		return aa;
  ...
  while(1){
    if(ds->packs){
      ...
      if(p->pts){
        ...
        if( ds == demux->audio ) 
        {
        	sh_audio_t *sa = (sh_audio_t *)ds->sh;
        	double cpts;
        	
        	
        	
        	printf("ds->pts = %8.3f\n", ds->pts); // ghcstop_caution
        }
        #endif
        ds->pts_bytes=0;
      }
...
//printf("ds_fill_buffer init\n");

SDL_LockMutex(ds->dsmutex);    
    if(ds->current) free_demux_packet(ds->current);
    //if(ds==demux->audio) printf("a\n"); // ghcstop for debug
    
    while(1)
    {
        if( ds->abort_request == 1) 
        {
            ret = 0;
            break;
        }
        ...
        {
        	SDL_CondWait(ds->dscond, ds->dsmutex);
        }
        ...
        if( ds == demux->audio )
        {
        	...
        	{
            	mp_msg(MSGT_DEMUXER,MSGL_ERR,MSGTR_TooManyAudioInBuffer,0,demux->audio->bytes);
            	mp_msg(MSGT_DEMUXER,MSGL_HINT,MSGTR_MaybeNI);
            	goto no_more_data;
        	}
        }
        else if( ds == demux->video )
        {
        	...
        	{
            	mp_msg(MSGT_DEMUXER,MSGL_ERR,MSGTR_TooManyVideoInBuffer,0,demux->video->bytes);
            	mp_msg(MSGT_DEMUXER,MSGL_HINT,MSGTR_MaybeNI);
            	goto no_more_data;
        	}
        }
...
no_more_data:
    ...
    ds->current=NULL;
    //mp_msg(MSGT_DEMUXER,MSGL_V,"ds_fill_buffer: EOF reached (stream: %s)  \n",ds==demux->audio?"audio":"video");
    printf("ds_fill_buffer: EOF reached (stream: %s)  \n",ds==demux->audio?"audio":"video");
    ds->eof=1;
...
#ifndef DFTE // ghcstop: original
inline float ds_get_next_pts(demux_stream_t *ds) {
  demuxer_t* demux = ds->demuxer;
  while(!ds->first) {
    if(demux->audio->packs>=MAX_PACKS || demux->audio->bytes>=MAX_PACK_BYTES){
      mp_msg(MSGT_DEMUXER,MSGL_ERR,MSGTR_TooManyAudioInBuffer,demux->audio->packs,demux->audio->bytes);
...
demuxer_t* demux_open(stream_t *vs,int file_format,int audio_id,int video_id,int dvdsub_id,char* filename){
  ...
  if(as) {
    ...
    else if(ad->audio->sh && ((sh_audio_t*)ad->audio->sh)->format == 0x55) 
