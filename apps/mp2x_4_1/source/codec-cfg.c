...
#include "libmpcodecs/img_format.h" // ghcstop fixed from libvo/img_format.h
#include "codec-cfg.h"
...
static int
add_to_fourcc(char *s, char *alias, unsigned int *fourcc, unsigned int *map)
{
    int             i,
                    j,
                    freeslots;
...
static int
add_to_format(char *s, char *alias, unsigned int *fourcc, unsigned int *fourccmap)
{
    int             i,
                    j;
    char           *endptr;
...
static int
add_to_inout(char *sfmt, char *sflags, unsigned int *outfmt, unsigned char *outflags)
{
...
#if 0
static short
get_driver(char *s, int audioflag)
{
...
static int
validate_codec(codecs_t * c, int type)
{
    unsigned int    i;
...
#    warning HOL VANNAK DEFINIALVA????????????
    if (!c->dll && (c->driver == 4 || (c->driver == 2 && type == TYPE_VIDEO)))
    {
...
get_token(int min, int max)
{
    ...
    for (i = 0; i < max; i++)
    {
        while (isspace(line[line_pos]))
            ++line_pos;
        if (line[line_pos] == '\0' || line[line_pos] == '#' || line[line_pos] == ';')
        {
            read_nextline = 1;
            if (i >= min)
                goto out_ok;
...
// ghcstop add 20041117: 나중에 init_best_audio_codec()에서 find_codec()등을 호출할때
// 참조하는 코덱 리스트를 등록하는 부분이다.
int
all_codec_register(void)
{
...
#ifndef CODECS2HTML             // codecs.conf.h를 만들기 위한 standalone일 경우는 builtin_video_codecs등이 없기
    // 때문에 이렇게 처리를 해줘야 한다.(왜냐하면 codecs.conf.h가 있어야 하기 때문이다.
    ...
    while ((tmp = get_token(1, 1)) != RET_EOF)
    {
        ...
        if (!strcmp(token[0], "audiocodec") || !strcmp(token[0], "videocodec"))
        {
          ...
          loop_enter:
            ...
            if (!(*codecsp = (codecs_t *) realloc(*codecsp, sizeof(codecs_t) * (*nr_codecsp + 2))))
            {
            ...
            {
                if (((*codecsp)[i].name != NULL) && (!strcmp(token[0], (*codecsp)[i].name)))
                {
        ...
        else if (!strcmp(token[0], "fourcc"))
        {
            if (get_token(1, 2) < 0)
                goto err_out_parse_error;
            if (!add_to_fourcc(token[0], token[1], codec->fourcc, codec->fourccmap))
                goto err_out_print_linenum;
        }
        else if (!strcmp(token[0], "format"))
        {
            if (get_token(1, 2) < 0)
                goto err_out_parse_error;
            if (!add_to_format(token[0], token[1], codec->fourcc, codec->fourccmap))
                goto err_out_print_linenum;
        }
        ...
        else if (!strcmp(token[0], "out"))
        {
            if (get_token(1, 2) < 0)
                goto err_out_parse_error;
            if (!add_to_inout(token[0], token[1], codec->outfmt, codec->outflags))
                goto err_out_print_linenum;
        }
        else if (!strcmp(token[0], "in"))
        {
            if (get_token(1, 2) < 0)
                goto err_out_parse_error;
            if (!add_to_inout(token[0], token[1], codec->infmt, codec->inflags))
                goto err_out_print_linenum;
        }
...
static void
codecs_free(codecs_t * codecs, int count)
{
    int             i;
...
codecs_t       *
find_audio_codec(unsigned int fourcc, unsigned int *fourccmap, codecs_t * start)
{
    return find_codec(fourcc, fourccmap, start, 1);
}

codecs_t       *
find_video_codec(unsigned int fourcc, unsigned int *fourccmap, codecs_t * start)
{
    return find_codec(fourcc, fourccmap, start, 0);
}

codecs_t       *
find_codec(unsigned int fourcc, unsigned int *fourccmap, codecs_t * start, int audioflag)
{
    int             i,
                    j;
    codecs_t       *c;
...
void
select_codec(char *codecname, int audioflag)
{
    int             i;
    codecs_t       *c;
...
void
codecs_reset_selection(int audioflag)
{
    int             i;
    codecs_t       *c;
...
#    ifdef HAVE_NEW_GUI
void
gtkMessageBox(int type, char *str)
{
    return;
}
...
void
parsehtml(FILE * f1, FILE * f2, codecs_t * codec, int section, int dshow)
{
    int             c,
                    d;
    ...
    while ((c = fgetc(f1)) >= 0)
    {
        ...
        case 'd':
            ...
            break;
        case 'D':
            fprintf(f2, "%c", !strcmp(codec->drv, "dshow") ? '+' : '-');
            break;
        case 'F':
...
static void
print_int_array(const int *a, int size)
{
    printf("{ ");
    while (size--)
        if (abs(*a) < 256)
...
static void
print_char_array(const unsigned char *a, int size)
{
    printf("{ ");
    while (size--)
        if ((*a) < 10)
...
main(int argc, char *argv[])
{
    ...
    if (argc > 1)
    {
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            for (j = 0; j < nr[i]; j++)
            {
                ...
                print_string(cod[i][j].name);
                printf(", /* name */\n");
                print_string(cod[i][j].info);
                printf(", /* info */\n");
                print_string(cod[i][j].comment);
                printf(", /* comment */\n");
                print_string(cod[i][j].dll);
                printf(", /* dll */\n");
                print_string(cod[i][j].drv);
                printf(", /* drv */\n");

                printf("{ 0x%08lx, %hu, %hu,", cod[i][j].guid.f1, cod[i][j].guid.f2, cod[i][j].guid.f3);
                print_char_array(cod[i][j].guid.f4, sizeof(cod[i][j].guid.f4));
                printf(" }, /* GUID */\n");
                printf("%hd /* flags */, %hd /* status */, %hd /* cpuflags */ }\n", cod[i][j].flags, cod[i][j].status, cod[i][j].cpuflags);
                if (j < nr[i])
                    printf(",\n");
            }
            printf("};\n\n");
    ...
    f1 = fopen("DOCS/tech/codecs-in.html", "rb");
    if (!f1)
        exit(1);
    f2 = fopen("DOCS/en/codecs-status.html", "wb");
    if (!f2)
        exit(1);
