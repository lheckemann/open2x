...
while(!sh_audio->inited && *audio_codec_list)
{
  ...
  if(audio_codec[0])
  {
    ...
    {
      //printf("init_best_audio_codec 1\n");
  ...
  else 
  {
    ...
    if(audio_fm_list)
    {
      ...
      {   
        ...
        //printf("init_best_audio_codec 4\n");
    ...
    if(!sh_audio->inited)
    {
        //printf("init_best_audio_codec 5\n"); // 여기를 거치는 군... 
	for(status=CODECS_STATUS__MAX;status>=CODECS_STATUS__MIN;--status)
...
int decode_audio(sh_audio_t *sh_audio,unsigned char *buf,int minlen,int maxlen)
{
  ...
  while(declen>sh_audio->a_buffer_len)
  {
      ...
      len=mpadec->decode_audio(sh_audio, sh_audio->a_buffer+sh_audio->a_buffer_len, len, maxlen);
      if(len<=0) break; // EOF?
