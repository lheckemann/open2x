...
static int control(int cmd,void *arg)
{
    switch(cmd)
    {
		...
		case AOCONTROL_HW_EQ:
		{
        	...
        	{
                ioctl(fd, MIXER_WRITE(SOUND_MIXER_EQ_HW), eqMode); 
            	close(fd);    
            }
            else 
            {
...
static int init(int rate,int channels,int format,int flags)
{
    ...
    if(mixer_channel)
    {
        ...
        {
            mp_msg(MSGT_AO,MSGL_ERR,MSGTR_AO_OSS_CantOpenMixer,oss_mixer_device, strerror(errno));
        }
        else
        {
            ioctl(fd, SOUND_MIXER_READ_DEVMASK, &devs);
            ...
            {
                mp_msg(MSGT_AO,MSGL_ERR,MSGTR_AO_OSS_ChanNotFound, mixer_channel);
            }
...
ac3_retry:  
    ...
    if( ioctl(audio_fd, SNDCTL_DSP_SETFMT, &ao_data.format)<0 || ao_data.format != format) 
    {
    	if(format == AFMT_AC3)
    	{
    ...
    printf("ao_data_buffer size = %d, 262144/o_bps = %f\n\n\n", ao_data.buffersize, 262144./(float)ao_data.bps);
...
static void reset()
{
//	printf("******* audio reset **********8\n");
	ioctl(audio_fd, SNDCTL_DSP_RESET, NULL);	// hhsong modify
/*
    uninit(1);
...
//[*]----------------------------------------------------------------------------------------------------[*]
#if 0
// front out buffer underflow check
static void audio_uf_chk(void)
{
	printf("******* audio underflow check **********\n");
	ioctl(audio_fd, SNDCTL_DSP_CONTROLLER_RESET, NULL);	// hhsong modify
}
#endif
//[*]----------------------------------------------------------------------------------------------------[*]
...
static int get_space()
{
  	int playsize=ao_data.outburst;
  		...
  		if(ioctl(audio_fd, SNDCTL_DSP_GETOSPACE, &zz)!=-1)
  		{
			...
			#else
      			if (playsize > 65536)
        			playsize = (65536 / zz.fragsize) * zz.fragsize;
        	#endif
...
static float get_delay()
{
  	...
  	if(audio_delay_method==1)
  	{
      	...
      	if(ioctl(audio_fd, SNDCTL_DSP_GETOSPACE, &zz)!=-1)
      	{
         	//printf("zz.bytes = %d, %f\n", zz.bytes, (float)(ao_data.buffersize-zz.bytes)/(float)ao_data.bps );
...
//[*]----------------------------------------------------------------------------------------------------[*]
#if 1 // 050118: 이 함수는 이제 사용안함
unsigned int get_delay_bytes()
{
        /* Calculate how many bytes/second is sent out */
        if(audio_delay_method==2)
        {
 ...
 //             printf("1-1\n");
        ...
        if(ioctl(audio_fd, SNDCTL_DSP_GETOSPACE, &zz)!=-1)
        {
                return (unsigned int)(ao_data.buffersize-zz.bytes);
        }
        audio_delay_method=0; // fallback if not supported
        }
