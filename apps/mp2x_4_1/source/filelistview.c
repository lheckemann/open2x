...
//senquack - added so we can reset the video mode when videos stop (to fix shifted screen bug)
...
//			break;
		case VK_START	:	
			...
			viewmode = MOVIE_VIEW;
			break;
		//senquack - probably should have a default case, eh?
		default :
			break;
	}
}
...
void FileView_OnMoveHighFolder(SDL_Surface *pScreen)
{
	...
	if(strcasecmp("/mnt", szPath) != 0)		
	{
    	...
    	for(i=0; i<infoDir.nCount; i++)
    	{
			...
			//	senquack - fixing a bug when going to the containing folder and the selection is off the screen
...
//			if(cpuclock >= CLOCKADJUST_COUNT)cpuclock = 0;
//			
//			//senquack - Crikey, GPH are retarded.. this probably never worked in the first place (because
//			//I have seen their /dev/GPIO ioctl interfce and it is utterly broken.  
