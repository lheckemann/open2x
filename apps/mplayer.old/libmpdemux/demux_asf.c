...
//#define DUMPF(a) {printf("0x%02X%02X%02X%02X\n", *(a++), *(a++),*(a++),*(a++));a-=4;}
...
int demux_asf_fill_buffer(demuxer_t *demux){
    ...
    verbose = 2;
    gprintf("asf: filepos = %u, start: %u, end: %u\n", demux->filepos, demux->movi_start, demux->movi_end);
    ...
    
    {
        ...
        gprintf("%08X:  flag=%02X  segs=%d  seq=%d  plen=%d  pad=%d  time=%ld  dur=%d\n",
          demux->filepos,flags,segs,sequence,plen,padding,time,duration);

		// ghcstop: for문 두번째서부터 문제가 생긴다. 아마도 seg 0일때 먼가 문제가 생김.
            ...
            switch(rlen)
            {
            	...
            	default:
              		...
              		else 
              		{
                  		...
                  		gprintf("unknown segment type (rlen): 0x%02X  \n",rlen);
                  		time2=0; // unknown
                  		p+=rlen;
              		}
            }
            ...
            switch(rlen)
            {
            	case 0x01:
              		...
              		if(len!=0)
              		{
                		...
                		gprintf("ASF_parser: warning! groups total != len\n");
              		}
              		break;
            	default:
              		// NO GROUPING:
...
int demux_asf_fill_buffer(demuxer_t *demux){
	int sv= verbose;
        ...
        unsigned char* p_end;
        unsigned char flags;
        volatile unsigned char segtype; // Dark050510 volatile declare
        int padding;
        int plen;
        int sequence;
        unsigned long time=0;
    ...
    {
        p=asf_packet;
        p_end=asf_packet+asf_packetsize;
        flags=p[0];
        segtype=p[1];
