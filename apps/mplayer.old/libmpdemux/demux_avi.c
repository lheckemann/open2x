...
// Select ds from ID
demux_stream_t *
demux_avi_select_stream(demuxer_t * demux, unsigned int id)
{
...
static int
choose_chunk_len(unsigned int len1, unsigned int len2)
{
...
static int
demux_avi_read_packet(demuxer_t * demux, demux_stream_t * ds, unsigned int id, unsigned int len, int idxpos, int flags)
{
    avi_priv_t     *priv = demux->priv;
    int             skip;
    float           pts = 0;
...
//          printf("\rYYY-V  A: %5.3f  V: %5.3f  \n",priv->avi_audio_pts,priv->avi_video_pts);
        ...
        //printf("vread  pack_no: %d  pts %5.3f  \n", demux->video->pack_no + demux->video->packs, pts);  // ghcstop
    }
...
int
demux_avi_fill_buffer(demuxer_t * demux)
{
    avi_priv_t     *priv = demux->priv;
    unsigned int    id = 0;
    unsigned int    len;
...
//int max_packs=128;
    ...
    do
    {
        ...
        if (priv->idx_size > 0 && priv->idx_pos < priv->idx_size)
        {
            ...
            //printf("1davi: [%d], id: %d\n",priv->idx_pos, idx->ckid);fflush(stdout);// ghcstop
...
//    if((len)!=(idx->dwChunkLength)){
        ...
        if (ds)
            if (ds->packs + 1 >= MAX_PACKS || ds->bytes + len >= MAX_PACK_BYTES)
            {
                ...
                #if 0
                if (ds == demux->audio)
                    printf("d_avi_a: %d,%d,%d,%X\n", ds->packs + 1, MAX_PACKS, ds->bytes + len, MAX_PACK_BYTES);
                else if (ds == demux->video)
                    printf("d_avi_v: %d,%d,%d,%X\n", ds->packs + 1, MAX_PACKS, ds->bytes + len, MAX_PACK_BYTES);
                #endif    
        ...
        #if 0 // ghcstop
        if (ds == demux->video)
            printf("or_d_avi_V: %d,%d,%d,%X\n", ds->packs, MAX_PACKS, ds->bytes, MAX_PACK_BYTES);
        else
            printf("or_d_avi_A: %d,%d,%d,%X\n", ds->packs, MAX_PACKS, ds->bytes, MAX_PACK_BYTES);              
        #endif    
...
//     1 = successfully read a packet
int
demux_avi_fill_buffer_ni(demuxer_t * demux, demux_stream_t * ds)
{
    avi_priv_t     *priv = demux->priv;
    unsigned int    id = 0;
    unsigned int    len;
...
//     1 = successfully read a packet
int
demux_avi_fill_buffer_nini(demuxer_t * demux, demux_stream_t * ds)
{
    avi_priv_t     *priv = demux->priv;
    unsigned int    id = 0;
    unsigned int    len;
...
char           *index_file_save = NULL,
    *index_file_load = NULL;
int             force_ni = 0;   // force non-interleaved AVI parsing

void            read_avi_header(demuxer_t * demuxer, int index_mode);

demuxer_t      *
demux_open_avi(demuxer_t * demuxer)
{
    ...
    if (priv->idx_size > 1)
    {
        ...
        mp_msg(MSGT_DEMUX, MSGL_V, "AVI index offset: 0x%X (movi=0x%X idx0=0x%X idx1=0x%X)\n",
               (int) priv->idx_offset, (int) demuxer->movi_start, (int) ((AVIINDEXENTRY *) priv->idx)[0].dwChunkOffset, (int) ((AVIINDEXENTRY *) priv->idx)[1].dwChunkOffset);
    }
//  demuxer->endpos=avi_header.movi_end;
    ...
    mp_msg(MSGT_DEMUX, MSGL_INFO, "VIDEO:  [%.4s]  %ldx%ld  %dbpp  %5.3f fps  %5.1f kbps (%4.1f kbyte/s)\n",
           (char *) &sh_video->bih->biCompression, sh_video->bih->biWidth, sh_video->bih->biHeight, sh_video->bih->biBitCount, sh_video->fps, sh_video->i_bps * 0.008f, sh_video->i_bps / 1024.0f);
...
void
demux_seek_avi(demuxer_t * demuxer, float rel_seek_secs, int flags)
{
...
void
demux_close_avi(demuxer_t * demuxer)
{
    avi_priv_t     *priv = demuxer->priv;
...
int
demux_avi_control(demuxer_t * demuxer, int cmd, void *arg)
{
    avi_priv_t     *priv = demuxer->priv;
