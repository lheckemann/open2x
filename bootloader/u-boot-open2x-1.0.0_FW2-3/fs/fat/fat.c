...
static long get_contents(fsdata *mydata, dir_entry *dentptr, __u8 *buffer, unsigned long maxsize)
{
	...
	// 우선 인덱스를 얻어온다.
	// 어디까기 얻어오냐 하면 0xFFFFFFFF(-1)가 나오거나 10000(128섹터)를 얻어온다.
	// 왜냐 하면 한번에 SD가 가지고 올수 있는 데이터량이 20000이 최고 이기때문에 
	// 안전하게 10000까지만 얻어 온다.
	do
	{
		// 여기서 파일 크기 이전이나 전체 읽을 용량이 10000 이하 일 경우 까지 체크
		...
		// 여기면 어라 0x10000보다 클 경우이므로 다 읽어 버린다.
		//printf("actsize %x\n", actsize);
		if(actsize >= 0x100000)
		{
			if(get_cluster(mydata, curclust, buffer, (int)actsize) != 0) 
...
static int get_vfatname(fsdata *mydata, int curclust, __u8 *cluster, dir_entry *retdent, char *l_name)
{
	...
	else if (*l_name == aRING) *l_name = 0xE5;
	downcase(l_name);
