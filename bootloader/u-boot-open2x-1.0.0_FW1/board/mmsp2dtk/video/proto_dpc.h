...
void DPC_CloseHardware(void);

//#define DPC_SetInterlace(IsInterlace)   {if(IsInterlace)DPC->DPC_CNTL|=INTRACE; else DPC->DPC_CNTL &= ~INTRACE}
//#define DPC_SetClockSource(ClockSource) (DPC->CLKSRC = ClockSource)
//#define DPC_SetCLKHPhase(Phase) 	(DPC->CHPHASE = Phase)
