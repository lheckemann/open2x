...
# endif	/* CONFIG_BOOT_RETRY_TIME */
	...
	if (SDK2X_KEY_START & sdk2x_SysReadKeys()) /* Check to see if the menu key is pressed */
	{
		...
		while (!selected && !cancelled)
		{
			...
			{
				change = 0;
				sdk2x_LcdTextOut (100, 50, "Boot from NAND", (option == 1)?0xF800:0xFFFF);
				sdk2x_LcdTextOut (100, 60, "Boot from SD/MMC",  (option == 2)?0xF800:0xFFFF);
				sdk2x_LcdTextOut (100, 70, "U-Boot Serial Prompt",  (option == 3)?0xF800:0xFFFF);
				sdk2x_LcdTextOut (100, 80, "Upgrade NAND Firmware",  (option == 4)?0xF800:0xFFFF);
				sdk2x_MSleep(20);
			}
		...
		if (!cancelled)
		{
			sdk2x_LcdTextOut (100, 50, "Boot from NAND", (option == 1)?0x07E0:0x0000);
			sdk2x_LcdTextOut (100, 60, "Boot from SD/MMC",  (option == 2)?0x07E0:0x0000);
			sdk2x_LcdTextOut (100, 70, "U-Boot Serial Prompt",  (option == 3)?0x07E0:0x0000);
			sdk2x_LcdTextOut (100, 80, "Upgrade NAND Firmware",  (option == 4)?0x07E0:0x0000);
			...
			switch (option)
			{
				...
				case 2:
					...
					if (0 != sdk2x_SdHwInit())
					{
						sdk2x_LcdTextOut (0, 90, "No FAT16/32 filesystem found on card", 0xF800);
						sdk2x_MSleep(5000);
					}
					else
					{
						...
						if (size < 0)
						{
							sdk2x_LcdTextOut (0, 90, "Failed to load Kernal.img from card.", 0xF800);
							sdk2x_MSleep(5000);
						}
						else
						{
