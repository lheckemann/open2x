...
#define BLK_DEV_RAM_SIZE	(8*1024)
...
#endif
#define VA_ARM940_BASE	0xf4000000
...
static __inline__ unsigned int read_gpio_bit(unsigned int num)
{
//	unsigned long addr = io_p2v(0xc0001180 + (0x2 * ((0xf0 & num) >> 4)));
	unsigned int value = __REGW(0xc0001180 + (0x2 * ((0xf0 & num) >> 4))) & 0xffff;
	unsigned long bit_ofs = num & 0xf;

	return ((value & (1 << bit_ofs)) >> bit_ofs);
}

static __inline__ void write_gpio_bit(unsigned int num, unsigned int val)
{
//	unsigned long addr = io_p2v(0xc0001060 + (0x2 * ((0xf0 & num) >> 4)));
	unsigned long addr = 0xc0001060 + (0x2 * ((0xf0 & num) >> 4));
	unsigned long bit_ofs = num & 0xf;
	unsigned short temp;
