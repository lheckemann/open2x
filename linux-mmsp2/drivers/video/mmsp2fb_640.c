...
static void mmsp2_lcd_init(unsigned long mmsp2_fbmem_addr)
{
    ...
    DISPCSET = (1 << 14) | (8 << 8) | (0 << 7); // 640 x 480
#elif defined(CONFIG_800x600_LCD)    
    // 이건 원래거 그냥 사용한거라 왕짜증 나네... 왜냐? 테스트 못해보거덩...
	DISPCSET = (0x1 << 14) | (12 << 7) | (0); // clk = (FPLL / (7+1))
#endif

	// mode setting, rgb666, interlace none
	DPC_CNTL = 0x0004;
	DPC_INTR = 0x0000;
	...
	DPC_HS_WIDTH     &= ~0x00ff;
	DPC_HS_WIDTH     |= 0x005f;	// T2
	DPC_HS_END        = (24/2) - 1;	// T7
	DPC_HS_STR        = (24/2) + (24%1) - 1; //T8
	DPC_DE           &= ~0x0ff0;
	DPC_DE           |= 0x0270;
	DPC_FPIPOL1      |= 0x0002;
