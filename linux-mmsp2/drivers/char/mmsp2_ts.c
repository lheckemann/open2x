...
M2DTK_SetTouchPanelControlOut(void)
{
	...
	set_gpio_ctrl(GPIO_C15, GPIOMD_OUT, GPIOPU_NOSET); // YMON
	set_gpio_ctrl(GPIO_C14, GPIOMD_OUT, GPIOPU_NOSET); // nXPON
	set_gpio_ctrl(GPIO_C13, GPIOMD_OUT, GPIOPU_NOSET); // nYPON
}
...
static ssize_t mmsp2_ts_read(struct file *filp, char *buf, size_t count, loff_t *l)
{
	...
	if(pen_data == PEN_DOWN) {
		...
		//printk("--> X Position[%04d], Y Position[%04d]\n", ts.x, ts.y);	
...
while (1) {
	unsigned short  xPosition, yPosition;
		...
		TPC_TimeDelay( 0x100 );
		udelay(10);
		xPosition = TPC_GetStaticADCOut( 1 );
		...
		TPC_TimeDelay( 0x100 );		
		udelay(10);
		yPosition = TPC_GetStaticADCOut( 0 );
