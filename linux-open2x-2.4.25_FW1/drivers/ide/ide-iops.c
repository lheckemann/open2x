...
static void ide_insw (unsigned long port, void *addr, u32 count)
{
	...
	if(!(read_gpio_bit(GPIO_D5)))
		return insb(port, addr, count<<1);
	else
		return insw(port, addr, count);
#elif defined(CONFIG_MACH_MMSP2_DTK3)
	if ((S1_STSR & S1_nCD)==0) /* byte access */
		return insb(port, addr, count<<1);
	else
		return insw(port, addr, count);
#endif
...
#ifdef CONFIG_MACH_MMSP2_DTK4
	if(!(read_gpio_bit(GPIO_D5)))
		return outsb(port, addr, count<<1);
	else
		return outsw(port, addr, count);
#elif defined(CONFIG_MACH_MMSP2_DTK3)
	if ((S1_STSR & S1_nCD)==0) /* byte access */
		outsb(port, addr, count<<1);
	else
...
#else
	{
		for (j=0; j<wcount/SECTOR_WORDS;j++)
		{
			while ((hwif->INB(IDE_STATUS_REG) & DRQ_STAT)==0);
			for (i=0; i<256;i++)
				*((volatile unsigned short*)buffer++) = hwif->INW(IDE_DATA_REG);
		}
	}
#endif
