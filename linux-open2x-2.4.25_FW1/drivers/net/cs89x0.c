...
#elif defined(CONFIG_ARCH_MMSP2)
#include <asm/hardware.h>
...
static unsigned	int netcard_portlist[] __initdata = { MDK_ETH_VIO_BASE+DEFAULTIOBASE, 0 };
#else
static unsigned	int netcard_portlist[] __initdata =
...
#ifdef CONFIG_ARCH_MMSP2 // irq setting, see asm/arch/mdk.h
        printk("0x%x\n", MDK_ETH_VIO_BASE+DEFAULTIOBASE);

#ifdef CONFIG_MACH_MMSP2_DTK3
        set_external_irq(IRQ_ETHER, EINT_HIGH_LEVEL, GPIOPU_EN);
		outw(PP_ChipID, ioaddr + ADD_PORT);
#elif defined(CONFIG_MACH_MMSP2_DTK4)
        set_external_irq(IRQ_ETHER, EINT_HIGH_LEVEL, GPIOPU_DIS);
		outw(PP_ChipID, ioaddr + ADD_PORT);
#endif 
...
net_open(struct	net_device *dev)
{
	...
	if (request_irq(dev->irq, net_interrupt, SA_INTERRUPT, "cs8900a", dev)) {
