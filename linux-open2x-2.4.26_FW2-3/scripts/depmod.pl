...
# Copyright (c) 2004 SeonKon Choi <bushi@mizi.com>
...
if (defined $opt{help}) {
	...
	"     -e --verbose\tPrint out lots of debugging stuff\n",
...
if (defined $opt{version}) {
	print
	"module-init-tools perl script for cross-devel\n";
	exit 0;
}
...
# Find the list of .ko files living under $basedir 
...
foreach $module (keys %$dep) {
    ...
    foreach (@{$dep->{$module}}) {
        if( $exp->{$_} ) { 
			...
			$symbol_name = $_;
			$_=$actual_path;
			s|/lib/modules|$basedir|;
			warn "unresolved symbol $symbol_name in file $_\n"
        }
    } 
}
...
# $Id: depmod.pl,v 1.1.4.1 2004/04/30 06:17:48 bushi Exp $
