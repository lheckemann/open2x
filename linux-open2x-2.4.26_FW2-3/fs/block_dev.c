...
static unsigned long max_block(kdev_t dev)
{
	...
	if (blk_size[major]) 
	{
...
#else
		unsigned int blocks,nbit=0;
		if( mmsp_get_usbCONN()) nbit=mmsp_get_sdsize();
		blocks = blk_size[major][minor] << nbit;
#endif
		...
		if (blocks) 
		{
			...
			size = block_size(dev) << nbit;
			sizebits = blksize_bits(size);
#endif
...
#else			
			if( mmsp_get_usbCONN()) bdev->bd_inode->i_size = blkdev_size(dev) << (mmsp_get_sdsize());			
			else bdev->bd_inode->i_size = blkdev_size(dev);	
#endif	
