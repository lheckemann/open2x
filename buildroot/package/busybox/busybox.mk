...
$(BUSYBOX_DIR)/.configured: $(BUSYBOX_DIR)/.unpacked $(BUSYBOX_CONFIG_FILE)
	cp -f package/busybox/busybox.config $(BUSYBOX_DIR)/.config
...
ifeq ($(BR2_PACKAGE_BUSYBOX_INSTALL_SYMLINKS),y)
	...
	cp -fr $(BUSYBOX_DIR)/_install/* $(TARGET_DIR)/
...
busybox-menuconfig: $(BUILD_DIR) busybox-source $(BUSYBOX_DIR)/.configured
	$(MAKE) __TARGET_ARCH=$(ARCH) -C $(BUSYBOX_DIR) menuconfig
	cp -f $(BUSYBOX_DIR)/.config package/busybox/busybox.config
