				 ...
				 struct jffs2_raw_inode *ri, uint32_t ofs)
{
	...
	if (!ic) {
		...
		if (crc != je32_to_cpu(ri->node_crc)) {
			D1(printk(KERN_NOTICE "jffs2_scan_inode_node(): CRC failed on node at 0x%08x: Read 0x%08x, calculated 0x%08x\n",
				  ...
				  struct jffs2_raw_dirent *rd, uint32_t ofs)
{
	...
	if (crc != je32_to_cpu(rd->node_crc)) {
		D1(printk(KERN_DEBUG "jffs2_scan_dirent_node(): Node CRC failed on node at 0x%08x: Read 0x%08x, calculated 0x%08x\n",
