...
//	printk("*[%s:%d]-%s : ino #%u, count(%d),to(%x),retlen(%d)\n", __FUNCTION__,__LINE__,__FILE__,ino,count,to,*retlen);
	...
	*/
	...
	printk("****[%s:%d]-%s : ino #%u, count(%d),to(%x),retlen(%d),c->wbuf_ofs(%x),c->wbuf_len(%x)\n", __FUNCTION__,__LINE__,__FILE__,ino,count,to,*retlen,c->wbuf_ofs,c->wbuf_len);
		if (c->wbuf_len)
