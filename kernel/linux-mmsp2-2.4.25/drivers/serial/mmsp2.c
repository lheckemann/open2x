 ...
 * $Revision$
   ...
   Tue Mar 16 2004 SeonKon Choi <bushi@mizi.com>
   - adopt linux-2.4.25-vrs2
...
{
	struct mmsp2_port *sport = (struct mmsp2_port *) port;
	//if (test_and_clear_bit(NUM_RX, &IRQ_BIT(sport)))
...
{
	struct mmsp2_port *sport = (struct mmsp2_port *) port;
	return (UART_UTRSTAT(sport) & UTRSTAT_TR_EMP ? TIOCSER_TEMT : 0);
}
...
//	.verify_port	= mmsp2uart_verify_port,      
};
...
#if 0
	mmsp2_ports[idx].port.uartclk = mmsp2_get_aclk()/10;

	switch (port) {
	case 0:
		mmsp2_ports[idx].port.membase = (void*)io_p2v(0xc0001200);
		mmsp2_ports[idx].port.mapbase = 0xc0001200;
		mmsp2_ports[idx].port.irq = IRQ_TXD0;
		mmsp2_ports[idx].port.line = idx;
		mmsp2_ports[idx].port.unused[0] = 0;
		//CLKCON |= CLKCON_UART0;
		break;
	case 1:
		mmsp2_ports[idx].port.membase = (void*)io_p2v(0xc0001220);
		mmsp2_ports[idx].port.mapbase = 0xc0001220;
		mmsp2_ports[idx].port.irq = IRQ_TXD1;
		mmsp2_ports[idx].port.line = idx;
		mmsp2_ports[idx].port.unused[0] = 0;
		//CLKCON |= CLKCON_UART1;
		break;
	case 2:
		mmsp2_ports[idx].port.membase = (void*)io_p2v(0xc0001240);
		mmsp2_ports[idx].port.mapbase = 0xc0001240;
		mmsp2_ports[idx].port.irq = IRQ_TXD2;
		mmsp2_ports[idx].port.line = idx;
		mmsp2_ports[idx].port.unused[0] = 0;
		//CLKCON |= CLKCON_UART2;
		break;
...
#if 0
static int
mmsp2_console_wait_key(struct console *co)
{
	int c;
	struct uart_port *port = mmsp2_ports + co->index;
