...
const char * mkyaffsimage_c_version = "$Id: mkyaffsimage.c,v 1.1.8.2 2003/12/01 09:09:51 bushi Exp $";

// External functions for ECC on data
//void nand_calculate_ecc (const u_char *dat, u_char *ecc_code);
void yaffs_ECCCalculate(const unsigned char *data,unsigned char *ecc);
#define nand_calculate_ecc(__dat, __code) yaffs_ECCCalculate(__dat, __code)
...
static void object_header_little_to_big_endian(yaffs_ObjectHeader* oh)
{
    ...
    oh->st_atime_ = SWAP32(oh->st_atime_);
    oh->st_mtime_ = SWAP32(oh->st_mtime_);
    oh->st_ctime_ = SWAP32(oh->st_ctime_);
#endif
...
// NCB 12/9/02		oh->st_gid = s->st_uid;
		oh->st_gid = s->st_gid;
		oh->st_atime_ = s->st_atime;
		oh->st_mtime_ = s->st_mtime;
		oh->st_ctime_ = s->st_ctime;
