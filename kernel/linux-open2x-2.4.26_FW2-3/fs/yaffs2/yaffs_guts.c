 ...
 *
 * Add support for holes in file, add write verify for YAFFS1
 * Copyright (c) Purple Labs SA 2005  <freesoftware@purplelabs.com> 
 * 
...
static int yaffs_HandleWriteChunkOk(yaffs_Device * dev, int chunkInNAND,
				     const __u8 * data,
	...
	do {
		...
		if (chunk >= 0) {
			...
			if (writeOk) {
				 ...
				 *  Todo
				 */
				writeOk = yaffs_HandleWriteChunkOk(dev, chunk, data,
							 tags);
			} else {
...
{
  int i,ret,errors = 0;
  yaffs_ExtendedTags read_tags;
  __u8 read_page[512];
  if ((ret = yaffs_ReadChunkWithTagsFromNAND(dev, chunkInNAND, read_page,  &read_tags))!=YAFFS_OK)
  {
    int blockInNAND = chunkInNAND / dev->nChunksPerBlock;
    T(YAFFS_TRACE_ERROR,(TSTR("**>> read back for write verify failed on chunk %d (error:%d)" TENDSTR),chunkInNAND,ret));
    /* Mark the block for retirement */
    yaffs_GetBlockInfo(dev, blockInNAND)->needsRetiring = 1;
    return YAFFS_FAIL;
  }
  for (i=0;i<512;i++)
    if (read_page[i]!=data[i]) errors++;
  if (errors)
  {
    int blockInNAND = chunkInNAND / dev->nChunksPerBlock;
    T(YAFFS_TRACE_ERROR,(TSTR("**>> write verify failed on chunk %d (%d errors)" TENDSTR),chunkInNAND,errors));
    /* Mark the block for retirement */
    yaffs_GetBlockInfo(dev, blockInNAND)->needsRetiring = 1;
    return YAFFS_FAIL;
  }
...
static int yaffs_CountChunks(yaffs_Device * dev, yaffs_Tnode * tn, __u32 level)
{
	int i;
	int total = 0;
	...
	if (tn) {
		if (level == 0) {
			...
			for (i = 0; i < YAFFS_NTNODES_INTERNAL; i++) {
				if (tn->internal[i]) 
					total += yaffs_CountChunks(dev, tn->internal[i], level - 1);
			}
		}
	}
	return total;
...
int yaffs_GetObjectChunkCount(yaffs_Object * obj)
{
  yaffs_Device *dev = obj->myDev;
  yaffs_FileStructure * fStruct = &obj->variant.fileVariant;

  return yaffs_CountChunks(dev, fStruct->top, fStruct->topLevel);
}
...
int yaffs_MakeHole(yaffs_Object * in, int offset,int size)
{
	int fileSize = in->variant.fileVariant.fileSize;
	yaffs_Device *dev = in->myDev;
	int i,chunkId;
	...
	/* Delete chunks in the hole */
	endChunk = 1 + (offset + size - dev->nBytesPerChunk) / dev->nBytesPerChunk;
	startChunk = 2 + (offset-1) / dev->nBytesPerChunk;
