...
static int yaffs_ioctl(struct inode *inode, struct file *filp,unsigned int cmd,unsigned long arg);
		...
		case YAFFS_HOLE:
			{
				...
				copy_from_user(&area,( void * ) arg,sizeof(hole_area_t));

				T(YAFFS_TRACE_OS,(KERN_DEBUG "yaffs_ioctl YAFFS_HOLE of object %d\n",
					yaffs_InodeToObject(inode)->objectId));

				dev = yaffs_InodeToObject(inode)->myDev;
				yaffs_GrossLock(dev);
				error = yaffs_MakeHole(yaffs_InodeToObject(inode),area.offset , area.size);
				yaffs_GrossUnlock(dev);
				return error;
			}
			break;
		case YAFFS_DISKSIZE:
			{
				...
				dev = yaffs_InodeToObject(inode)->myDev;
				yaffs_GrossLock(dev);
				size = dev->nBytesPerChunk * yaffs_GetObjectChunkCount(yaffs_InodeToObject(inode));
				if (put_user(size,(int *)arg))
					return -EFAULT;
				yaffs_GrossUnlock(dev);
				return 0;
			}
