...
//------------------------------------------------------------------------------
//	MP3 DECODER PARAMETERS
//------------------------------------------------------------------------------
// DEC_FRAME : Send Stream information to ARM940T
typedef struct _DEC_FRAME_
{
	...
	unsigned short 	_dumy[1];

	unsigned int	InMP3DataSize;
	unsigned long	InMP3DataAddr;

	unsigned long	OutPCMDataAddr;

} MP3DecFrame, Mp3DecFrame;

// DEC_RESULT : Get PCM information from ARM940T
typedef struct _DEC_RESULT_
{
	unsigned short	Command;
	unsigned short	CodecType;

	unsigned short  SubCommand;
	unsigned short 	_dumy1[1];

	unsigned int	OutPCMDataSize;
	unsigned long   OutPCMDataAddr;

	int				errorcode;

} MP3DecResult, Mp3DecResult;

// StreamInfo : Get MP3 information from ARM940T
typedef struct {
    int     sampleRate;
    int     numChannels;
    int     Layer;
...
/* MP3 Decode IOCTL Commands */
#	define IOCTL_DUALCPU_MP3D_SEND_DECFRAME		_IOW('d', 0x40, MP3DecFrame)
#	define IOCTL_DUALCPU_MP3D_GET_DECRESULT		_IOW('d', 0x41, MP3DecResult)
#	define IOCTL_DUALCPU_MP3D_GET_MP3STREAMINFO	_IOW('d', 0x42, MP3StreamInfo)
