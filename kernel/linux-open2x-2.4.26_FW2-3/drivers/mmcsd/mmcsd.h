...
struct mmcsd_cmd {
	...
	__u32 nSectors; /* sector of count */
...
#ifndef CONFIG_GP2X_SD_CARD
	/* transfer 1 block for Memory Card */
//	int (*transfer1b)(struct mmcsd_slot *slot, int rd, u_long from, u_char *buf);
#else
	/* transfer Mult block code function */
	int (*transferMultblock)(struct mmcsd_slot *slot, int rd, u_long from, u_char *buf,u_long nSectors);
#endif
