 ...
 *
*************************************************************************/
static int mmcsd_do_transfer_Multblock(struct mmcsd_slot *slot, int rd, u_long from, u_char *buf,u_long nSectors)
{
	int ret;
	struct mmcsd_cmd cmd;
...
//		if( (slot->read_len > 512) || (slot->write_len > 512) )
	...
	if (rd)
	{
		...
		if(nSectors < 2) cmd.cmd = MMCSD_CMD17; /* single read */
		else cmd.cmd = MMCSD_CMD18;				/* mult read */

		cmd.nSectors = nSectors;

		cmd.arg = from;
		cmd.res_type = MMCSD_RES_TYPE_R1;
		cmd.res_flag = MMCSD_RES_FLAG_RDATA;
		cmd.data = buf;							/* req->buffer*/
		...
		if (ret < 0)
		{
			printk("CMD%d failed, ret = %d\n",(nSectors < 2 ) ? 17:18 , ret);
	...
	else
	{
		if(slot->card_in)
		{
			...
			if(nSectors < 2) cmd.cmd = MMCSD_CMD24;
			else cmd.cmd = MMCSD_CMD25;
...
#ifdef CONFIG_GP2X_SD_CARD
	slot->transferMultblock = mmcsd_do_transfer_Multblock;
#else
...
int del_mmcsd_device(struct mmcsd_slot *slot)
{
	...
	i = sys_umount("/mnt/sd", MNT_FORCE);
	printk("del_mmcsd_device: umount /mnt/sd...%d\n",i);
