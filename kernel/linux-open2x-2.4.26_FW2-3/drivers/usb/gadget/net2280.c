...
#define ISNET2282	// 2282 version of chip, undef for 2280
...
#ifdef ISNET2282	
#define	DRIVER_DESC		"NetChip 2282 USB Peripheral Controller"
#else
...
net2280_enable (struct usb_ep *_ep, const struct usb_endpoint_descriptor *desc)
{
	...
	else // Added for 2282, Don't use nak packets on an in endpoint, this was ignored on 2280
	{
	...
	while (!list_empty (&ep->queue)) {
		...
		if (unlikely (req->td->dmadesc == 0)) {
			...
			if ((tmp & (1 << NAK_OUT_PACKETS)) == 0) {
				// As a test see if we are waiting for the dma to complete
				tmp = readl (&ep->regs->ep_avail);
				if (tmp == 0)
				{
				...
				{
					// wait for dma to complete, see 2282 section 8.4
					int i;
					for (i = 0; i < 10; i++)
					{
						udelay(1);
						...
						if (tmp == 0)
						{
							tmp = readl (&ep->dma->dmacount) & DMA_BYTE_COUNT_MASK;		
							printk("Test shows interrupt called before dma complete!\n");
							break;
						}
					}
					if (tmp > 0)
					{
						WARN (ep->dev, "DMA didn't complete data left in ep %s!\n",
						      ep->ep.name);
						req->req.status = -EOVERFLOW;						
					}
				} // end 2282 test code
	...
	if (likely (ep->dma != 0)) {
		if (t & (1 << SHORT_PACKET_TRANSFERRED_INTERRUPT)) {
			...
			if (likely (req != 0)) {
				...
				// short packet completion happening here?
