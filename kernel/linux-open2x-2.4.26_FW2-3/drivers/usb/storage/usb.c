...
extern char usb_dev_name [];
extern asmlinkage long sys_umount(char * name, int flags);
static int usb_umount (char *dev)
{
	char *envp[] = { "HOME=/", "TERM=linux", "PATH=/sbin:/usr/sbin:/bin:/usr/bin", NULL };
	char *argv[] = { "/bin/umount", "-f", NULL, NULL };
	char path[32];
	char str[128];
	char *tok;
	int ret, result;
	...
	while(tok != NULL) {
		sprintf(path, "/mnt/ext/%s", tok);
		argv[2] = path;

		ret = sys_umount(path, 0);
		printk("usb-storage: sys_umount %s, errno = %d\n", path, ret);
		ret = call_usermodehelper(argv[0], argv, envp);
		if (ret) {
			printk(KERN_ERR
						 "usb-storage: failed to call %s %s, errno = %d\n", argv[0], argv[2], ret);
			result = ret;
		}
...
#ifdef CONFIG_HOTPLUG
		strcpy(usb_dev_name, ss->sd_name);
#else
...
#ifdef CONFIG_HOTPLUG
		strcpy(ss->sd_name, usb_dev_name);
#endif
...
#ifdef CONFIG_HOTPLUG
	strcpy(usb_dev_name, ss->sd_name);
	removed = usb_umount(usb_dev_name);
	printk("storage_disconnect: usb_remove %s...%d\n", usb_dev_name, removed); // NK Check
#endif
