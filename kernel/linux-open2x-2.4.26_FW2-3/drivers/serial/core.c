...
extern void mmsp2_console_init(void);
...
uart_report_port(struct uart_driver *drv, struct uart_port *port)
{
	...
	switch (port->iotype) {
	case UPIO_PORT:
		printk(" at I/O 0x%x", port->iobase);
		break;
	case UPIO_HUB6:
		printk(" at I/O 0x%x offset 0x%x", port->iobase, port->hub6);
		break;
	case UPIO_MEM:
		printk(" at MMIO 0x%lx", port->mapbase);
		break;
	}
		    ...
		    struct uart_port *port)
{
	...
	if (port->type != PORT_UNKNOWN) {
		...
		uart_report_port(drv, port);	/* MSG FUNCTION*/
#endif
		/*
