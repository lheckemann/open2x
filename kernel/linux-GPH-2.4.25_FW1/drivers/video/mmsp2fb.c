...
#ifdef CONFIG_MACH_GP2X
	ClkInfo.DISPCLK_DIVIDER= 20;   //10	//old 15
#else
...
#elif CONFIG_MACH_MMSP2_DTK4
    DPC_UTIL_HVSYNC (DPC_RGB_666, LCD_WIDTH, LCD_HEIGHT, /*5, 5, 16,*/ 96, 24, 40, CTRUE, /*2, 4, 6,*/ 2, 10, 33, CTRUE);
#endif
#ifdef CONFIG_MACH_GP2X
...
static int mmsp2fb_ioctl(struct inode *inode, struct file *file, u_int cmd, u_long arg, int con, struct fb_info *info)
{
    ...
    switch( MSG(pdmsg) )
    {
        case MMSP2_FB_RGB_ON:
			...
			//printk("PLANE0: on\n");  
			// src width, src height, stride(=width*(bpp/8)), dst width, dst, height
