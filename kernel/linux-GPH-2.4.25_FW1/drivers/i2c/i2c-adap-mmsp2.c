...
static void i2c_mmsp2_stop(void)
{
	...
	IICCON &= ~I2C_INTPEND_FLAG;	// interrupt pend flag를 지운다.
	udelay(100);
}
...
** <Description>
** i2c_int_type == TXRX_FINISHED : return 0 --> data 송/수신 완료.
**                 BUS_ARBIT_FAIL : return BUS_ERROR --> bus error 발생.
** timeout < 0, --> return -EIO --> timeout error 발생.
