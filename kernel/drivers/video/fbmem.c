...
extern int mmsp2fb_init(void);
extern int mmsp2fb1_init(void);
...
static struct {
	...
	{ "g364", g364fb_init, NULL },
        ...
        { "mmsp2", mmsp2fb_init, NULL },
        { "mmsp2_1", mmsp2fb1_init, NULL },        
#endif
	 ...
	 unsigned long arg)
{
	...
	case FBMMSP2CTRL:
		if (fb->fb_ioctl == NULL)
			return -EINVAL;
		return fb->fb_ioctl(inode, file, cmd, arg, PROC_CONSOLE(info), info);
