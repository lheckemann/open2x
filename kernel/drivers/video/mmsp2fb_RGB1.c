 ...
 *
 * original linux/drivers/video/s3c2410fb.c
 */
...
static int mmsp2fb1_ioctl(struct inode *inode, struct file *file, u_int cmd, u_long arg, int con, struct fb_info *info)
{
    ...
    switch( MSG(pdmsg) )
    {
        ...
        case MMSP2_FB_RGB_ON:
			...
			if(GetTVCheck())
			{
				if(GetTvMode())
				{
					...
					MLC_RGB_SetCoord
					(
						MLC_RGB_RGN_2,PLANE_X_OFFSET, MMSP2_FB_TV_PAL_WIDTH,
				...
				else
				{
					...
					MLC_RGB_SetCoord
					(
						MLC_RGB_RGN_2,PLANE_X_OFFSET, MMSP2_FB_TV_NTSC_WIDTH,
        ...
        case MMSP2_FB_RGB_ALPHA:
			MLC_RGB_MixMux(MLC_RGB_RGN_2, MLC_RGB_MIXMUX_ALPHA, pdmsg->msgdata[0] & 0xf);
            MSG(pdmsg) = MSGOK;
            LEN(pdmsg) = 0;
        	break;
