 ...
 *	performance
 *
 * $Id: nand.c,v 1.81 2004/04/16 04:12:00 gleixner Exp $
 *
...
#ifdef DS_DEBUG
#  define ds_printk(x...) printk(KERN_INFO x)
#else
	...
	u_char *oob_buf, struct nand_oobinfo *oobsel, int chipnr, int oobmode)
{
	...
	for(;;) {	
		...
		if (this->verify_buf(mtd, &this->data_poi[datidx], mtd->oobblock)) {
			...
			ds_printk("1111 Failed write verify, page 0x%08x\n ",page);
			goto out;
		}
		datidx += mtd->oobblock;
		...
		if (oobmode) {
			if (this->verify_buf(mtd, &oob_buf[oobofs], mtd->oobsize)) {
				DEBUG (MTD_DEBUG_LEVEL0, "%s: " "Failed write verify, page 0x%08x ", __FUNCTION__, page);
			ds_printk("2222 Failed write verify, page 0x%08x\n ",page);
				goto out;
			}
		} else {
			...
			if (oobsel->useecc != MTD_NANDECC_OFF) {
				...
				ds_printk("3333 Failed write verify, page 0x%08x, %6i bytes were succesful\n ",page,i);
						goto out;
					}
				}
			}	
...
#ifndef CONFIG_MTD_NAND_MP2520F
			while (!this->dev_ready(mtd));	
#else
		{
			int limit = 1000;
...
//		printk("nand_read_ecc: aligned(%d), len=%i, read=%i, chipnr(%d)\n", aligned, (int) len, (int)read,chipnr);
		if (!page) {
			chipnr++;
			this->select_chip(mtd, -1);
			this->select_chip(mtd, chipnr);
