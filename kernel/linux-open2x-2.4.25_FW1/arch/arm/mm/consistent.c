...
//	printk("** %s : dma_handle = 0x%x \n",__FUNCTION__,(unsigned int)dma_handle);
	ret = __ioremap(page_to_pfn(page) << PAGE_SHIFT, size, 0);
//	printk("** %s : virtual addr = 0x%x \n",__FUNCTION__, (unsigned int)ret);
	if (!ret)
		goto no_remap;
...
void consistent_free(void *vaddr, size_t size, dma_addr_t handle)
{
	 ...
	 */
	size = PAGE_ALIGN(size);
//	printk("** %s : vaddr = 0x%x, size = %d, handle = 0x%x \n",__FUNCTION__, (unsigned int)vaddr,size,(unsigned int)handle);
