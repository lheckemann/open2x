...
gadgetfs_bind (struct usb_gadget *gadget)
{
	...
	printk("gadgetfs_bind\n");
	if (!dev)
	{
		printk(KERN_ERR "%s device null\n", shortname);
		return -ESRCH;
	}
	INFO (dev, "Binding to %s driver\n", gadget->name);	
	...
	if (!dev->req)
	{
		printk(KERN_ERR "%s usb_ep_alloc_request\n", shortname);		
		goto enomem;
	}
	...
	{
		printk(KERN_ERR "%s activate_ep_files\n", shortname);		
		goto enomem;
	}
...
dev_config (struct file *fd, const char __user *buf, size_t len, loff_t *ptr)
{
	...
	spin_unlock_irq (&dev->lock);
	printk("Registering with gadget api\n");
	value = usb_gadget_register_driver (&gadgetfs_driver);
	if (value != 0) {
		printk("usb_gadget_register_driver failed\n");
		kfree (dev->buf);
		dev->buf = NULL;
	} else {
		...
		struct dentry **dentry_p)
{
	...
	if (!dentry)
	{
		printk(KERN_ERR "Couldn't allocate %s\n", name);
		return NULL;
	}
...
gadgetfs_get_sb (struct super_block *sb, void *data, int silent)
{
	if (!gadgetfs_fill_super (sb, data, silent))
	{
		printk("Created super block!\n");
		return the_device->sb;
	}
	else return NULL;
}
