...
//senquack - fixed some blitting offsets and sizes here:
int MessageBox::exec() {
	int result = -1;
...
//	box.h = gmenu2x->font->getHeight()*3 +4;
	box.h = gmenu2x->font->getHeight()*3 +14;
...
//	bg.rectangle(box.x+2, box.y+2, box.w-4, box.h-gmenu2x->font->getHeight(), gmenu2x->skinConfColors["messageBoxBorder"]);
	bg.rectangle(box.x+2, box.y+2, box.w-4, box.h-gmenu2x->font->getHeight()-10, gmenu2x->skinConfColors["messageBoxBorder"]);
	//icon+text
...
//			buttonPositions[i].y = box.y+box.h-4;
			buttonPositions[i].y = box.y+box.h-12;
